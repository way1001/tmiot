...
import com.aiforest.tmiot.common.exception.OutRangeException;
import com.aiforest.tmiot.common.utils.ArithmeticUtil;
import lombok.*;
...
public class RValue implements Serializable {
    ...
    public String getFinalValue() {
        ...
        Integer bitwise = Optional.ofNullable(point.getBitwise()).orElse((Integer) 0);
        ...
        return switch (valueType) {
            ...
            case BOOLEAN -> String.valueOf(getBooleanValue(value, bitwise));
        };
    }
     ...
     * @return boolean
     */
    private boolean getBooleanValue(String rawValue, Integer bitwise) {
        if (rawValue == null || bitwise == null || bitwise < 0) {
            return false; // 非法输入默认返回false
        }
        try {
            // 1. 将字符串转为整数（支持十进制、十六进制或二进制）
            long number;
            if (rawValue.startsWith("0x")) {
                number = Long.parseLong(rawValue.substring(2), 16); // 十六进制
            } else if (rawValue.matches("[01]+")) {
                number = Long.parseLong(rawValue, 2); // 二进制
            } else {
                number = Long.parseLong(rawValue); // 十进制
            }

            // 2. 检查指定位是否为1
            return (number & (1L << bitwise)) != 0;
        } catch (NumberFormatException e) {
            // 3. 若解析失败，尝试直接作为布尔字符串处理
